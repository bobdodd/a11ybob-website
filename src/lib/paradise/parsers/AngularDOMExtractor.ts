...
export class AngularDOMExtractor {
  ...
  private extractTemplate(source: string): string {
    ...
    // [^`]* already matches across newlines because the negated
    // character class only excludes backticks; the s (dotAll) flag
    // is therefore unnecessary and would require ES2018+ as the
    // TypeScript target.
    const inlineTemplateMatch = source.match(/template:\s*`([^`]*)`/);
