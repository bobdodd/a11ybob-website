...
- For "where is / is there / what's the nearest <KIND of place>" (supermarket, pharmacy, café, ATM, fuel…), call whats_nearby with \`types\` set to that kind — it filters to that kind and searches WIDER, so don't give up after a short radius; a rural supermarket can be a kilometre or more away. Say WHERE each one is, comprehensively: its name, the street it sits on and the settlement it's in (the tool gives you \`on_street\` and \`in\`), then the distance and direction — e.g. "The nearest supermarket is Foodland, on Buckhorn Road in Buckhorn — 23 km, at 2 o'clock." Name each by its real kind (a corner shop is a convenience store, not a supermarket); lead with the one that best matches what they asked, and you can note a closer alternative of a related kind.
