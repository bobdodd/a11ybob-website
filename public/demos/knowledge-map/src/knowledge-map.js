/* Knowledge Map (v2) — the Conversational Map's chat, plus a knowledge layer.
 *
 * The user asks (typed OR spoken) about where they are or anywhere on the map; we POST to
 * /api/knowledge-chat (the LLM tool loop — the same map tools as the Conversational Map,
 * plus place_knowledge for what a place is KNOWN for) and read the answer back. Voice INPUT
 * goes through /api/context-stt (Deepgram, shared); spoken OUTPUT uses Web Speech, with an
 * ARIA-live fallback for devices without a voice (de-Googled phones). A clone of the
 * Conversational Map's front-end: disclaimer gate, location + compass, navigable transcript,
 * a text input, and a Speak button. */
...
start.addEventListener("click", () => {
  ...
  speak("Ready. Ask where you are, or about anywhere on the map — and I can tell you what a place is known for, too. Type it, or use the Speak button.");
});
...
async function startListen() {
  ...
  try {
    ...
    await sttCtx.audioWorklet.addModule("/demos/knowledge-map/src/pcm-worklet.js");
